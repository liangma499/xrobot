...
func (gb *ethInfo) BnbCreateAndSendTrx(privateKeyStr string, toAddress, contractAddress string, amount int64, gasPlaces, contractPlaces int8) (string, string, error) {
	...
	minBalanceGas := xtypes.CoefficientInt64(decimal.NewFromFloat(accountBalance), gasPlaces).BigInt()
	if contractAddress == "-" || contractAddress == "" {
		...
		if balance.Cmp(minBalanceGas) < 0 {
			return "", fromADdressStr, fmt.Errorf("insufficient funds:fromAddress:%s balance %s,minBalanceGas:%s wei", fromADdressStr, balance.String(), minBalanceGas.String())
		}
		//可用余额
		minNeed := new(big.Int).Set(balance).Sub(balance, minBalanceGas)
		// 检查是否有足够的余额
		...
		log.Warnf("BEP_20 fromAddress:%s,balance:%s  minNeed:%s txCost:%s totalCost:%s wei\n",
			fromADdressStr,
			balance.String(),
			minNeed.String(),
			txCost.String(),
		...
		totalCost := new(big.Int).Set(txCost).Add(txCost, minBalanceGas)
		log.Warnf("BEP_20 contractAddress:%s ,fromAddress:%s,balance: %s  spl :%s minNeed:%s txCost:%s totalCost:%s  wei\n",
			contractAddress,
			fromADdressStr,
			balance.String(),
