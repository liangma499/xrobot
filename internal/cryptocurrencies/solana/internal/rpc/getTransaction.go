...
func (cl *Client) GetTransaction(
	ctx context.Context,
	txSig internal.Signature, // transaction signature
...
// TransactionResultEnvelope will contain a *internal.Transaction if the requested encoding is `internal.EncodingJSON`
// (which is also the default when the encoding is not specified),
// or a `internal.Data` in case of EncodingBase58, EncodingBase64.
...
func (dt *TransactionResultEnvelope) GetData() internal.Data {
	return dt.asDecodedBinary
}

// GetRawJSON returns a *internal.Transaction when the data
// encoding is EncodingJSON.
func (dt *TransactionResultEnvelope) GetTransaction() (*internal.Transaction, error) {
