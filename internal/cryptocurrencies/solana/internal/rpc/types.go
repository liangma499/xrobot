...
type BlockhashResult struct {
	Blockhash     internal.Hash `json:"blockhash"`
...
type GetConfirmedBlockResult struct {
	...
	PreviousBlockhash internal.Hash `json:"previousBlockhash"`

	ParentSlot   uint64                    `json:"parentSlot"`
	Transactions []TransactionWithMeta     `json:"transactions"`
	Signatures   []internal.Signature      `json:"signatures"`
	Rewards      []BlockReward             `json:"rewards"`
	BlockTime    *internal.UnixTimeSeconds `json:"blockTime,omitempty"`
}

type BlockReward struct {
	// The public key of the account that received the reward.
	Pubkey internal.PublicKey `json:"pubkey"`
...
type TransactionWithMeta struct {
	...
	BlockTime *internal.UnixTimeSeconds `json:"blockTime" bin:"optional"`
...
func (dt TransactionWithMeta) GetParsedTransaction() (*internal.Transaction, error) {
	if dt.Transaction == nil {
		return nil, fmt.Errorf("transaction is nil")
	}
	if dt.Transaction.rawDataEncoding != internal.EncodingJSONParsed {
		return nil, fmt.Errorf("data is not in JSONParsed encoding")
	}
	var parsedTransaction internal.Transaction
...
func (twm TransactionWithMeta) MustGetTransaction() *internal.Transaction {
...
func (twm TransactionWithMeta) GetTransaction() (*internal.Transaction, error) {
	tx := new(internal.Transaction)
...
type TokenBalance struct {
	...
	Owner *internal.PublicKey `json:"owner,omitempty"`
	// Pubkey of token program.
	ProgramId *internal.PublicKey `json:"programId,omitempty"`

	// Pubkey of the token's mint.
	Mint          internal.PublicKey `json:"mint"`
...
type LoadedAddresses struct {
	ReadOnly internal.PublicKeySlice `json:"readonly"`
	Writable internal.PublicKeySlice `json:"writable"`
}
...
type ReturnData struct {
	ProgramId internal.PublicKey `json:"programId"`
...
type InnerInstruction struct {
	...
	Instructions []internal.CompiledInstruction `json:"instructions"`
}

// Ok  any `json:"Ok"`  // <null> Transaction was successful
// Err any `json:"Err"` // Transaction failed with TransactionError
...
type TransactionSignature struct {
	...
	Signature internal.Signature `json:"signature"`
...
type Account struct {
	...
	Owner internal.PublicKey `json:"owner"`
...
type DataBytesOrJSON struct {
	rawDataEncoding internal.EncodingType
...
func (dt DataBytesOrJSON) MarshalJSON() ([]byte, error) {
	if dt.rawDataEncoding == internal.EncodingJSONParsed || dt.rawDataEncoding == internal.EncodingJSON {
...
func (wrap *DataBytesOrJSON) UnmarshalJSON(data []byte) error {
	...
	case '{':
		...
		// TODO: is it always JSON???
		{
			...
			wrap.asJSON = data
			wrap.rawDataEncoding = internal.EncodingJSONParsed
		}
	default:
...
type GetConfirmedSignaturesForAddress2Opts struct {
	...
	Before     internal.Signature `json:"before,omitempty"`
	Until      internal.Signature `json:"until,omitempty"`
	Commitment CommitmentType     `json:"commitment,omitempty"`
}
...
type RPCFilterMemcmp struct {
	...
	Bytes  internal.Base58 `json:"bytes"`
}
...
type ParsedMessageAccount struct {
	PublicKey internal.PublicKey `json:"pubkey"`
...
type ParsedInstruction struct {
	...
	Data        internal.Base58          `json:"data,omitempty"`
	Accounts    []internal.PublicKey     `json:"accounts,omitempty"`
...
type Info struct {
	Destination internal.PublicKey `json:"destination,omitempty"` //"destination": "84YKYKo7qN54VHFLn6Eo5uBZMKzUY5Q9qB2t1L3drUeQ",
	Source      internal.PublicKey `json:"source,omitempty"`      //		"lamports": 836,
	Lamports    decimal.Decimal    `json:"lamports,omitempty"`    //"source": "8Juu8gXPHCKougXvkg3ZY8HdJcBdqFch7hHKjKFNWTFV"
	Account     internal.PublicKey `json:"account,omitempty"`     //"account":   "BMnsyyG6S6zkaE3K5X3nbRMKdvBS5dT6HhcMozBVL7Ly",
	Amount      string             `json:"amount,omitempty"`      //"amount":   // "47444666",
	Authority   internal.PublicKey `json:"authority,omitempty"`   //"authority": "7oPa2PHQdZmjSPqvpZN7MQxnC7Dcf3uL4oLqknGLk2S3",
	Mint        internal.PublicKey `json:"mint,omitempty"`        //"mint":      "E942z7FnS7GpswTvF5Vggvo7cMTbvZojjLbFgsrDVff1"
}
