...
func Test_AccountSubscribe(t *testing.T) {
	...
	c, err := Connect(context.Background(), "ws://api.mainnet-beta.internal.com:80")
	defer c.Close()
	require.NoError(t, err)

	accountID := internal.MustPublicKeyFromBase58("SqJP6vrvMad5XBQK5PCFEZjeuQSFi959sdpqtSNvnsX")
...
func Test_AccountSubscribeWithHttpHeader(t *testing.T) {
	...
	c, err := ConnectWithOptions(context.TODO(), "ws://api.mainnet-beta.internal.com:80", opt)
	defer c.Close()
	require.NoError(t, err)
...
func Test_ProgramSubscribe(t *testing.T) {
	...
	programID := internal.MustPublicKeyFromBase58("EUqojwWA2rd19FZrzeBncJsm38Jm1hEhE3zsmX3bRc2o")
