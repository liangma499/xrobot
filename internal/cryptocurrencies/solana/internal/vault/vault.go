...
type Vault struct {
	...
	KeyBag []internal.PrivateKey `json:"-"`
}
...
func NewVaultFromSingleKey(privKey string) (*Vault, error) {
	v := NewVault()
	key, err := internal.PrivateKeyFromBase58(privKey)
	if err != nil {
...
func (v *Vault) NewKeyPair() (pub internal.PublicKey, err error) {
	privKey, err := internal.NewRandomPrivateKey()
	if err != nil {
		return internal.PublicKey{}, err
	}
...
func (v *Vault) AddPrivateKey(privateKey internal.PrivateKey) internal.PublicKey {
