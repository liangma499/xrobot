...
var tokenRegistryRegisterCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) (err error) {
		...
		var tokenAddress internal.PublicKey
		if tokenAddress, err = internal.PublicKeyFromBase58(args[0]); err != nil {
		...
		registrarPubKey, err := internal.PublicKeyFromBase58(pkeyStr)
		if err != nil {
		...
		tokenMetaAccount := internal.NewWallet()
		...
		trx, err := internal.NewTransaction(
			[]internal.Instruction{
				createAccountInstruction,
				registerTokenInstruction,
			},
			blockHashResult.Value.Blockhash,
			internal.TransactionPayer(registrarPubKey),
		)
		if err != nil {
			return fmt.Errorf("unable to craft transaction: %w", err)
		}

		_, err = trx.Sign(func(key internal.PublicKey) *internal.PrivateKey {
