...
var vaultAddCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			log.Errorf("unable to load vault file: %v", err)
			return
		}

		boxer, err := vault.SecretBoxerForType(v.SecretBoxWrap, viper.GetString("global-kms-gcp-keypath"))
		if err != nil {
			log.Errorf("unable to intiate boxer: %v", err)
			return
		}

		err = v.Open(boxer)
		if err != nil {
			log.Errorf("unable to open vault: %v", err)
			return
		}
		...
		if err != nil {
			log.Errorf("failed to enter private keys: %v", err)
			return
		}

		var newKeys []internal.PublicKey
		...
		err = v.Seal(boxer)
		if err != nil {
			log.Errorf("failed to seal vault: %v", err)
			return
		}

		err = v.WriteToFile(walletFile)
		if err != nil {
			log.Errorf("failed to write vault file: %v", err)
			return
		}
...
func capturePrivateKeys() (out []internal.PrivateKey, err error) {
	fmt.Println("")
...
func capturePrivateKey(isFirst bool) (privateKey internal.PrivateKey, err error) {
	...
	key, err := internal.PrivateKeyFromBase58(enteredKey)
	if err != nil {
...
func vaultWrittenReport(walletFile string, newKeys []internal.PublicKey, totalKeys int) {
	fmt.Println("")
