...
// Vote account
func (inst *Withdraw) SetVoteAccount(voteAccount internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[0] = internal.Meta(voteAccount).WRITE()
	return inst
}

// Recipient account
func (inst *Withdraw) SetRecipientAccount(recipientAccount internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[1] = internal.Meta(recipientAccount).WRITE()
	return inst
}

// Withdraw authority account
func (inst *Withdraw) SetWithdrawAuthorityAccount(withdrawAccount internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[2] = internal.Meta(withdrawAccount).WRITE().SIGNER()
	return inst
}

func (inst *Withdraw) GetVoteAccount() *internal.AccountMeta      { return inst.AccountMetaSlice[0] }
func (inst *Withdraw) GetRecipientAccount() *internal.AccountMeta { return inst.AccountMetaSlice[1] }
func (inst *Withdraw) GetWithdrawAuthorityAccount() *internal.AccountMeta {
...
func NewWithdrawInstructionBuilder() *Withdraw {
	nd := &Withdraw{
		AccountMetaSlice: make(internal.AccountMetaSlice, 3),
	}
	return nd
}
...
func NewWithdrawInstruction(
	...
	lamports uint64,
	// Accounts:
	voteAccount internal.PublicKey,
	recipientAccount internal.PublicKey,
	withdrawAuthAccount internal.PublicKey,
) *Withdraw {
