...
type AddressLookupTableState struct {
	...
	Authority                  *internal.PublicKey
	Addresses                  internal.PublicKeySlice
}
...
func (a *AddressLookupTableState) UnmarshalWithDecoder(decoder *bin.Decoder) (err error) {
	...
	if serializedAddressesNumBytes == 0 {
		...
		a.Addresses = make(internal.PublicKeySlice, 0)
		return nil
	}
	...
	a.Addresses = make(internal.PublicKeySlice, numSerializedAddresses)

	for i := 0; i < numSerializedAddresses; i++ {
		var address internal.PublicKey
...
func NewKeyedAddressLookupTable(key internal.PublicKey) *KeyedAddressLookupTable {
