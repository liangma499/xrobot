...
func TestFetchMarket(t *testing.T) {
	...
	if rpcURL == "" {
		t.Skip("Setup 'RPC_URL' to run test i.e. 'https://api.mainnet-beta.internal.com'")
		return
	}
	...
	openOrderAdd, err := internal.PublicKeyFromBase58("jFoHUkNDC767PyK11cZM4zyNcpjLqFnSjaqEYp5GVBr")
...
func TestStreamOpenOrders(t *testing.T) {
	rpcURL := os.Getenv("RPC_URL")
	if rpcURL == "" {
		t.Skip("Setup 'RPC_URL' to run test i.e. 'wss://api.mainnet-beta.internal.com'")
		return
	}
