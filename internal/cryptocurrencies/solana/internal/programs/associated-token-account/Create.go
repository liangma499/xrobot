...
type Create struct {
	Payer  internal.PublicKey `bin:"-" borsh_skip:"true"`
	Wallet internal.PublicKey `bin:"-" borsh_skip:"true"`
	Mint   internal.PublicKey `bin:"-" borsh_skip:"true"`
...
func (inst *Create) SetPayer(payer internal.PublicKey) *Create {
	inst.Payer = payer
	return inst
}

func (inst *Create) SetWallet(wallet internal.PublicKey) *Create {
	inst.Wallet = wallet
	return inst
}

func (inst *Create) SetMint(mint internal.PublicKey) *Create {
	inst.Mint = mint
	return inst
}
...
func (inst Create) Build() *Instruction {
	...
	associatedTokenAddress, _, _ := internal.FindAssociatedTokenAddress(
		inst.Wallet,
		inst.Mint,
	)

	keys := []*internal.AccountMeta{
		{
		...
		{
			PublicKey:  internal.SystemProgramID,
			IsSigner:   false,
			IsWritable: false,
		},
		{
			PublicKey:  internal.TokenProgramID,
			IsSigner:   false,
			IsWritable: false,
		},
		{
			PublicKey:  internal.SysVarRentPubkey,
			IsSigner:   false,
			IsWritable: false,
		},
	}
...
func (inst *Create) Validate() error {
	...
	_, _, err := internal.FindAssociatedTokenAddress(
		inst.Wallet,
		inst.Mint,
	)
	if err != nil {
...
func NewCreateInstruction(
	payer internal.PublicKey,
	walletAddress internal.PublicKey,
	splTokenMintAddress internal.PublicKey,
) *Create {
