...
func TestEncDec(t *testing.T) {
	...
	{
		payerPrivateKey := internal.MustPrivateKeyFromBase58("5LRLfrUP22VtiNaPGAEgHPucoJmG8ejmomMVmpn4fkXjexYsT7RQGfGuMePG5PKvecZxMGrqa6EP2RmYcm7TYQvX")
		payerAccount, _ := internal.WalletFromPrivateKeyBase58(payerPrivateKey.String())
		programID := "4sCcZNQR8vfWckyi5L9KdptdaiLxdiMjVgKQay7HxzmK"
		programPubKey := internal.MustPublicKeyFromBase58(programID)

		newSubAccount, err := internal.CreateWithSeed(
...

		{
			...
			got.AccountMetaSlice = internal.AccountMetaSlice{
				internal.Meta(payerAccount.PublicKey()).WRITE().SIGNER(),
				internal.Meta(newSubAccount).WRITE(),
				internal.Meta(payerAccount.PublicKey()).SIGNER(),
			}
			ag_require.NoError(t, err)
