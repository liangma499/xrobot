// Copyright 2024 github.com/cordialsys
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//	http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package stake

import (
	"errors"
	"fmt"

	"tron_robot/internal/cryptocurrencies/solana/internal"
	bin "tron_robot/internal/cryptocurrencies/solana/internal/binary"

	"tron_robot/internal/cryptocurrencies/solana/internal/text/format"

	"github.com/gagliardetto/treeout"
)

type Withdraw struct {
	// Withdraw unstaked lamports from the stake account
	Lamports *uint64
	// [0] = [WRITE] Stake Account
	// ··········· Stake account from which to withdraw
	//
	// [1] = [WRITE] Recipient Account
	// ··········· Recipient account
	//
	// [2] = [] Clock Sysvar
	// ··········· The Clock Sysvar Account
	//
	// [3] = [] Stake History Sysvar
	// ··········· The Stake History Sysvar Account
	//
	// [4] = [SIGNER] Withdraw Authority
	// ··········· Withdraw authority
	//
	// OPTIONAL:
	// [5] = [SIGNER] Lockup authority
	// ··········· If before lockup expiration
	internal.AccountMetaSlice `bin:"-" borsh_skip:"true"`
}

func (inst *Withdraw) Validate() error {
	{
		if inst.Lamports == nil {
			return errors.New("lamports parameter is not set")
		}
	}
	// Check whether all accounts are set:
	for accIndex, acc := range inst.AccountMetaSlice {
		if acc == nil {
			return fmt.Errorf("ins.AccountMetaSlice[%v] is not set", accIndex)
		}
	}
	return nil
}
func (inst *Withdraw) SetStakeAccount(stakeAccount internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[0] = internal.Meta(stakeAccount).WRITE()
	return inst
}
func (inst *Withdraw) SetRecipientAccount(recipient internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[1] = internal.Meta(recipient).WRITE()
	return inst
}
func (inst *Withdraw) SetClockSysvar(clockSysvar internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[2] = internal.Meta(clockSysvar)
	return inst
}
func (inst *Withdraw) SetStakeHistorySysvar(historySysvar internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[3] = internal.Meta(historySysvar)
	return inst
}

func (inst *Withdraw) SetWithdrawAuthority(withdrawAuthority internal.PublicKey) *Withdraw {
	inst.AccountMetaSlice[4] = internal.Meta(withdrawAuthority).SIGNER()
	return inst
}

func (inst *Withdraw) GetStakeAccount() *internal.AccountMeta       { return inst.AccountMetaSlice[0] }
func (inst *Withdraw) GetRecipientAccount() *internal.AccountMeta   { return inst.AccountMetaSlice[1] }
func (inst *Withdraw) GetClockSysvar() *internal.AccountMeta        { return inst.AccountMetaSlice[2] }
func (inst *Withdraw) GetStakeHistorySysvar() *internal.AccountMeta { return inst.AccountMetaSlice[3] }
func (inst *Withdraw) GetWithdrawAuthority() *internal.AccountMeta  { return inst.AccountMetaSlice[4] }

func (inst *Withdraw) SetLamports(lamports uint64) *Withdraw {
	inst.Lamports = &lamports
	return inst
}

func (inst *Withdraw) UnmarshalWithDecoder(dec *bin.Decoder) error {
	{
		err := dec.Decode(&inst.Lamports)
		if err != nil {
			return err
		}
	}
	return nil
}

func (inst *Withdraw) MarshalWithEncoder(encoder *bin.Encoder) error {
	{
		err := encoder.Encode(*inst.Lamports)
		if err != nil {
			return err
		}
	}
	return nil
}

func (inst Withdraw) Build() *Instruction {
	return &Instruction{BaseVariant: bin.BaseVariant{
		Impl:   inst,
		TypeID: bin.TypeIDFromUint32(Instruction_Withdraw, bin.LE),
	}}
}

func (inst *Withdraw) EncodeToTree(parent treeout.Branches) {
	parent.Child(format.Program(ProgramName, ProgramID)).
		//
		ParentFunc(func(programBranch treeout.Branches) {
			programBranch.Child(format.Instruction("Withdraw")).
				//
				ParentFunc(func(instructionBranch treeout.Branches) {
					// Parameters of the instruction:
					instructionBranch.Child("Params").ParentFunc(func(paramsBranch treeout.Branches) {
						paramsBranch.Child(format.Param("Lamports", inst.Lamports))
					})

					// Accounts of the instruction:
					instructionBranch.Child("Accounts").ParentFunc(func(accountsBranch treeout.Branches) {
						accountsBranch.Child(format.Meta("                   StakeAccount", inst.AccountMetaSlice.Get(0)))
						accountsBranch.Child(format.Meta("               RecipientAccount", inst.AccountMetaSlice.Get(1)))
						accountsBranch.Child(format.Meta("                    ClockSysvar", inst.AccountMetaSlice.Get(2)))
						accountsBranch.Child(format.Meta("             StakeHistorySysvar", inst.AccountMetaSlice.Get(3)))
						accountsBranch.Child(format.Meta("              WithdrawAuthority", inst.AccountMetaSlice.Get(4)))
					})
				})
		})
}

// NewWithdrawInstructionBuilder creates a new `Withdraw` instruction builder.
func NewWithdrawInstructionBuilder() *Withdraw {
	nd := &Withdraw{
		AccountMetaSlice: make(internal.AccountMetaSlice, 5),
	}
	return nd
}

// NewWithdrawInstruction declares a new Withdraw instruction with the provided parameters and accounts.
func NewWithdrawInstruction(
	// Params:
	lamports uint64,
	// Accounts:
	stakeAccount internal.PublicKey,
	recipient internal.PublicKey,
	withdrawAuthority internal.PublicKey,
) *Withdraw {
	return NewWithdrawInstructionBuilder().
		SetLamports(lamports).
		SetStakeAccount(stakeAccount).
		SetRecipientAccount(recipient).
		SetClockSysvar(internal.SysVarClockPubkey).
		SetStakeHistorySysvar(internal.SysVarStakeHistoryPubkey).
		SetWithdrawAuthority(withdrawAuthority)
}
