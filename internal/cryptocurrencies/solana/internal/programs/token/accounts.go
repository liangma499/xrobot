...
type Mint struct {
	...
	MintAuthority *internal.PublicKey `bin:"optional"`
	...
	FreezeAuthority *internal.PublicKey `bin:"optional"`
}
...
func (mint *Mint) UnmarshalWithDecoder(dec *bin.Decoder) (err error) {
	{
		...
		if v == 1 {
			...
			mint.MintAuthority = internal.PublicKeyFromBytes(v).ToPointer()
		} else {
			// discard:
	...
	}
	{
		...
		if v == 1 {
			...
			mint.FreezeAuthority = internal.PublicKeyFromBytes(v).ToPointer()
		} else {
			// discard:
			_, err := dec.ReadNBytes(32)
			if err != nil {
...
type Account struct {
	...
	Mint internal.PublicKey

	// The owner of this account.
	Owner internal.PublicKey
	...
	Delegate *internal.PublicKey `bin:"optional"`
	...
	CloseAuthority *internal.PublicKey `bin:"optional"`
}
...
func (mint *Account) UnmarshalWithDecoder(dec *bin.Decoder) (err error) {
	{
		...
		mint.Mint = internal.PublicKeyFromBytes(v)
	}
	{
		v, err := dec.ReadNBytes(32)
		if err != nil {
		...
		mint.Owner = internal.PublicKeyFromBytes(v)
	}
	{
		v, err := dec.ReadUint64(binary.LittleEndian)
		if err != nil {
	...
	}
	{
		...
		if v == 1 {
			...
			mint.Delegate = internal.PublicKeyFromBytes(v).ToPointer()
		} else {
			// discard:
			_, err := dec.ReadNBytes(32)
			if err != nil {
	...
	}
	{
		...
		if v == 1 {
			...
			mint.CloseAuthority = internal.PublicKeyFromBytes(v).ToPointer()
		} else {
			// discard:
			_, err := dec.ReadNBytes(32)
			if err != nil {
...
type Multisig struct {
	...
	IsInitialized bool
	// Signer public keys
	Signers [MAX_SIGNERS]internal.PublicKey
}
