...
func Program(name string, programID internal.PublicKey) string {
...
func Account(name string, pubKey internal.PublicKey) string {
	return Shakespeare(name) + ": " + text.ColorizeBG(pubKey.String())
}

func MetaIfSetByIndex(name string, metaSlice internal.AccountMetaSlice, index int) string {
	if metaSlice == nil {
...
func Meta(name string, meta *internal.AccountMeta) string {
	if meta == nil {
