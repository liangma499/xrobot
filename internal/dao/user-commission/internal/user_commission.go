...
var userCommissionColumns = &Columns{
	UID:                 "uid", // 主键
...
func NewUserCommission(db *gorm.DB) *UserCommission {
	dao := &UserCommission{}
	dao.Columns = userCommissionColumns
	dao.TableName = "user_commission"
	dao.Database = db
	dao.Table = db.Model(&modelpkg.UserCommission{}).Table(dao.TableName)

	return dao
}

// New create a new instance and return
func (dao *UserCommission) New(tx *gorm.DB) *UserCommission {
	d := &UserCommission{}
	d.Columns = dao.Columns
	d.TableName = dao.TableName
	d.Database = tx
	d.Table = tx.Model(&modelpkg.UserCommission{}).Table(d.TableName)

	return d
}

// Insert executes an insert command to insert multiple documents into the collection.
func (dao *UserCommission) Insert(ctx context.Context, models ...*modelpkg.UserCommission) (int64, error) {
	if len(models) == 0 {
...
func (dao *UserCommission) Delete(ctx context.Context, filterFunc ...FilterFunc) (int64, error) {
	...
	rst := db.Delete(&modelpkg.UserCommission{})

	return rst.RowsAffected, rst.Error
}

// Update executes an update command to update documents in the collection.
func (dao *UserCommission) Update(ctx context.Context, filterFunc FilterFunc, updateFunc UpdateFunc) (int64, error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *UserCommission) Count(ctx context.Context, filterFunc ...FilterFunc) (count int64, err error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *UserCommission) Sum(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (sums []float64, err error) {
...
func (dao *UserCommission) Avg(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (avgs []float64, err error) {
	columns := columnFunc(dao.Columns)
	if len(columns) == 0 {
		return
	}
...
func (dao *UserCommission) First(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserCommission, error) {
	var (
		model = &modelpkg.UserCommission{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserCommission) Last(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserCommission, error) {
	var (
		model = &modelpkg.UserCommission{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserCommission) FindOne(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserCommission, error) {
	var (
		model = &modelpkg.UserCommission{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserCommission) FindMany(ctx context.Context, filterFunc FilterFunc, columnFunc ColumnFunc, orderFunc OrderFunc, limitAndOffset ...int) ([]*modelpkg.UserCommission, error) {
	var (
		models = make([]*modelpkg.UserCommission, 0)
		db     = dao.Table.WithContext(ctx)
	)
