...
type Columns struct {
	ID        string // 主键
	ZeroTime  string // 登录时间
	UID       string // 用户ID
	LoginTime string // 登录次数
}

var userLoginDayStatColumns = &Columns{
	ID:        "id",         // 主键
	ZeroTime:  "zero_time",  // 登录时间
	UID:       "uid",        // 用户ID
	LoginTime: "login_time", // 登录次数
}

func NewUserLoginDayStat(db *gorm.DB) *UserLoginDayStat {
	dao := &UserLoginDayStat{}
	dao.Columns = userLoginDayStatColumns
	dao.TableName = "user_login_day_stat"
	dao.Database = db
	dao.Table = db.Model(&modelpkg.UserLoginDayStat{}).Table(dao.TableName)

	return dao
}

// New create a new instance and return
func (dao *UserLoginDayStat) New(tx *gorm.DB) *UserLoginDayStat {
	d := &UserLoginDayStat{}
	d.Columns = dao.Columns
	d.TableName = dao.TableName
	d.Database = tx
	d.Table = tx.Model(&modelpkg.UserLoginDayStat{}).Table(d.TableName)

	return d
}

// Insert executes an insert command to insert multiple documents into the collection.
func (dao *UserLoginDayStat) Insert(ctx context.Context, models ...*modelpkg.UserLoginDayStat) (int64, error) {
	if len(models) == 0 {
...
func (dao *UserLoginDayStat) Delete(ctx context.Context, filterFunc ...FilterFunc) (int64, error) {
	...
	rst := db.Delete(&modelpkg.UserLoginDayStat{})

	return rst.RowsAffected, rst.Error
}

// Update executes an update command to update documents in the collection.
func (dao *UserLoginDayStat) Update(ctx context.Context, filterFunc FilterFunc, updateFunc UpdateFunc) (int64, error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *UserLoginDayStat) Count(ctx context.Context, filterFunc ...FilterFunc) (count int64, err error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *UserLoginDayStat) Sum(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (sums []float64, err error) {
...
func (dao *UserLoginDayStat) Avg(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (avgs []float64, err error) {
	columns := columnFunc(dao.Columns)
	if len(columns) == 0 {
		return
	}
...
func (dao *UserLoginDayStat) First(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserLoginDayStat, error) {
	var (
		model = &modelpkg.UserLoginDayStat{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserLoginDayStat) Last(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserLoginDayStat, error) {
	var (
		model = &modelpkg.UserLoginDayStat{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserLoginDayStat) FindOne(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserLoginDayStat, error) {
	var (
		model = &modelpkg.UserLoginDayStat{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserLoginDayStat) FindMany(ctx context.Context, filterFunc FilterFunc, columnFunc ColumnFunc, orderFunc OrderFunc, limitAndOffset ...int) ([]*modelpkg.UserLoginDayStat, error) {
	var (
		models = make([]*modelpkg.UserLoginDayStat, 0)
		db     = dao.Table.WithContext(ctx)
	)
