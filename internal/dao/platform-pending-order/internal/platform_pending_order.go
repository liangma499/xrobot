...
var platformPendingOrderColumns = &Columns{
	ID:           "id",            // 主键
	UID:          "uid",           // 主键
	Code:         "code",          // 编号
	ChannelCode:  "channel_code",  // 渠道编码
	Address:      "address",       
	ToCurrency:   "to_currency",   // 币种
	ToAmount:     "to_amount",     
	FromCurrency: "from_currency", // 币种
...
func NewPlatformPendingOrder(db *gorm.DB) *PlatformPendingOrder {
	dao := &PlatformPendingOrder{}
	dao.Columns = platformPendingOrderColumns
	dao.TableName = "platform_pending_order"
	dao.Database = db
	dao.Table = db.Model(&modelpkg.PlatformPendingOrder{}).Table(dao.TableName)

	return dao
}

// New create a new instance and return
func (dao *PlatformPendingOrder) New(tx *gorm.DB) *PlatformPendingOrder {
	d := &PlatformPendingOrder{}
	d.Columns = dao.Columns
	d.TableName = dao.TableName
	d.Database = tx
	d.Table = tx.Model(&modelpkg.PlatformPendingOrder{}).Table(d.TableName)

	return d
}

// Insert executes an insert command to insert multiple documents into the collection.
func (dao *PlatformPendingOrder) Insert(ctx context.Context, models ...*modelpkg.PlatformPendingOrder) (int64, error) {
	if len(models) == 0 {
...
func (dao *PlatformPendingOrder) Delete(ctx context.Context, filterFunc ...FilterFunc) (int64, error) {
	...
	rst := db.Delete(&modelpkg.PlatformPendingOrder{})

	return rst.RowsAffected, rst.Error
}

// Update executes an update command to update documents in the collection.
func (dao *PlatformPendingOrder) Update(ctx context.Context, filterFunc FilterFunc, updateFunc UpdateFunc) (int64, error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *PlatformPendingOrder) Count(ctx context.Context, filterFunc ...FilterFunc) (count int64, err error) {
    db := dao.Table.WithContext(ctx)
...
func (dao *PlatformPendingOrder) Sum(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (sums []float64, err error) {
...
func (dao *PlatformPendingOrder) Avg(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (avgs []float64, err error) {
	columns := columnFunc(dao.Columns)
	if len(columns) == 0 {
		return
	}
...
func (dao *PlatformPendingOrder) First(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.PlatformPendingOrder, error) {
	var (
		model = &modelpkg.PlatformPendingOrder{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *PlatformPendingOrder) Last(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.PlatformPendingOrder, error) {
	var (
		model = &modelpkg.PlatformPendingOrder{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *PlatformPendingOrder) FindOne(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.PlatformPendingOrder, error) {
	var (
		model = &modelpkg.PlatformPendingOrder{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *PlatformPendingOrder) FindMany(ctx context.Context, filterFunc FilterFunc, columnFunc ColumnFunc, orderFunc OrderFunc, limitAndOffset ...int) ([]*modelpkg.PlatformPendingOrder, error) {
	var (
		models = make([]*modelpkg.PlatformPendingOrder, 0)
		db     = dao.Table.WithContext(ctx)
	)
