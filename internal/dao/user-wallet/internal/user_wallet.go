...
type Columns struct {
	...
	UID              string // 用户ID
	Currency         string // 币种
	WalletAmountKind string
	Cash             string // 现金
	Used             string // 奖金
	Def              string // 是否默认
}

var userWalletColumns = &Columns{
	ID:               "id",       // 主键
	UID:              "uid",      // 用户ID
	Currency:         "currency", // 币种
	WalletAmountKind: "wallet_amount_kind",
	Cash:             "cash", // 现金
	Used:             "used", // 奖金
	Def:              "def",  // 是否默认
}
...
func NewUserWallet(db *gorm.DB) *UserWallet {
	dao := &UserWallet{}
	dao.Columns = userWalletColumns
	dao.TableName = "user_wallet"
	dao.Database = db
	dao.Table = db.Model(&modelpkg.UserWallet{}).Table(dao.TableName)

	return dao
}

// New create a new instance and return
func (dao *UserWallet) New(tx *gorm.DB) *UserWallet {
	d := &UserWallet{}
	d.Columns = dao.Columns
	d.TableName = dao.TableName
	d.Database = tx
	d.Table = tx.Model(&modelpkg.UserWallet{}).Table(d.TableName)

	return d
}

// Insert executes an insert command to insert multiple documents into the collection.
func (dao *UserWallet) Insert(ctx context.Context, models ...*modelpkg.UserWallet) (int64, error) {
	if len(models) == 0 {
...
func (dao *UserWallet) Delete(ctx context.Context, filterFunc ...FilterFunc) (int64, error) {
	...
	rst := db.Delete(&modelpkg.UserWallet{})

	return rst.RowsAffected, rst.Error
}

// Update executes an update command to update documents in the collection.
func (dao *UserWallet) Update(ctx context.Context, filterFunc FilterFunc, updateFunc UpdateFunc) (int64, error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *UserWallet) Count(ctx context.Context, filterFunc ...FilterFunc) (count int64, err error) {
	db := dao.Table.WithContext(ctx)
...
func (dao *UserWallet) Sum(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (sums []float64, err error) {
...
func (dao *UserWallet) Avg(ctx context.Context, columnFunc ColumnFunc, filterFunc ...FilterFunc) (avgs []float64, err error) {
	columns := columnFunc(dao.Columns)
	if len(columns) == 0 {
		return
	}
...
func (dao *UserWallet) First(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserWallet, error) {
	var (
		model = &modelpkg.UserWallet{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserWallet) Last(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserWallet, error) {
	var (
		model = &modelpkg.UserWallet{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserWallet) FindOne(ctx context.Context, filterFunc FilterFunc, columnFunc ...ColumnFunc) (*modelpkg.UserWallet, error) {
	var (
		model = &modelpkg.UserWallet{}
		db    = dao.Table.WithContext(ctx)
	)
...
func (dao *UserWallet) FindMany(ctx context.Context, filterFunc FilterFunc, columnFunc ColumnFunc, orderFunc OrderFunc, limitAndOffset ...int) ([]*modelpkg.UserWallet, error) {
	var (
		models = make([]*modelpkg.UserWallet, 0)
		db     = dao.Table.WithContext(ctx)
	)
