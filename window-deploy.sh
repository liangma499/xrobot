...
# 二进制执行目录
readonly bin_dir='E:/game-run/cr_vn_bin'
readonly bin_dir_bin='E:/game-run'
readonly bin_dir_bin_pkg='./cr_vn_bin'
# 启动环境
readonly env='prd'
# 临时编译目录
...
make_to_local() {
  ...
  'all')
    ...
    copy_script_to_local
    cd ${bin_dir_bin}
    tar czvf cr-vn-bin-$(date "+%Y%m%d%H%M%S").tar.gz  ${bin_dir_bin_pkg}/*
    ;;
  *)
    make_one_to_local "$1"
    copy_script_to_local
    cd ${bin_dir_bin}
    dirName=$(echo "$1" | tr 'A-Z' 'a-z')
    tar czvf cr-vn-$dirName-bin-$(date "+%Y%m%d%H%M%S").tar.gz  ${bin_dir_bin_pkg}/$dirName/*
    ;;
  esac
}
...
make_to_remote() {
  ...
  'all')
    make_all_to_remote
    cd ${bin_dir}
    tar czvf cr_vn-bin-$(date "+%Y-%m-%d-%H-%M-%S").tar.gz  ./*
    copy_script_to_remote
    ;;
  *)
